...
-- ============================================================
-- PHASE 1: Life Operating System foundation
-- ============================================================

-- Enums --------------------------------------------------------
DO $$ BEGIN CREATE TYPE public.item_rarity AS ENUM ('common','rare','epic','legendary','mythic');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN CREATE TYPE public.item_kind AS ENUM ('badge','artifact','boost','cosmetic','title','medal','book');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN CREATE TYPE public.focus_mode AS ENUM ('pomodoro','deep_work','custom');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN CREATE TYPE public.boss_status AS ENUM ('active','completed','failed','expired');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE POLICY "own user_titles" ON public.user_titles FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE INDEX user_titles_user_idx ON public.user_titles(user_id);

-- Add equipped title on profile
...
CREATE POLICY "own life_score_snapshots" ON public.life_score_snapshots FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "own weekly_bosses" ON public.weekly_bosses FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
-- Life Score (0-100): weighted composite of activity
...
DECLARE
  ...
  v_workouts_7d int := 0;
  v_reading_7d int := 0;
  v_habits_done_7d int := 0;
  v_focus_min_7d int := 0;
  v_recovery_days int := 0;
  v_relapse_7d int := 0;
  v_missions_7d int := 0;
  v_sleep_avg numeric := 0;
  s_xp numeric; s_streak numeric; s_missions numeric; s_habits numeric;
  s_workouts numeric; s_recovery numeric; s_reading numeric; s_focus numeric;
  s_sleep numeric; s_level numeric;
  v_total numeric;
BEGIN
  ...
  SELECT COUNT(*) INTO v_workouts_7d FROM public.workouts
    WHERE user_id = v_uid AND workout_date >= CURRENT_DATE - 7;
  SELECT COALESCE(SUM(pages_read),0) INTO v_reading_7d FROM public.health_logs
    WHERE user_id = v_uid AND log_date >= CURRENT_DATE - 7;
  ...
  SELECT COALESCE(SUM(GREATEST(0, floor(EXTRACT(EPOCH FROM (now() - started_at))/86400)::int)),0)
    INTO v_recovery_days FROM public.bad_habits
  ...
  s_reading  := LEAST(10, v_reading_7d::numeric / 100.0 * 10);
  ...
  s_sleep    := CASE WHEN v_sleep_avg = 0 THEN 5
                     ...
                     WHEN v_sleep_avg BETWEEN 6 AND 10 THEN 7
                     ELSE 4 END;

  v_total := (s_level + s_xp + s_streak + s_missions + s_habits + s_workouts + s_recovery + s_reading + s_focus + s_sleep);
...
BEGIN
  ...
  IF v_inserted THEN
    ...
    PERFORM public.record_timeline(p_user_id, 'title:'||p_key, 'title',
      'Título desbloqueado: '||p_name, p_desc, p_icon,
      jsonb_build_object('rarity', p_rarity));
  END IF;
...
-- Check + auto-award ALL titles based on real user state
...
DECLARE
  ...
  v_recovery_days int; v_habit_max int; v_journal int;
  v_awarded int := 0;
BEGIN
  IF v_uid IS NULL THEN RETURN 0; END IF;
  SELECT * INTO v_profile FROM public.profiles WHERE id = v_uid;
  ...
  SELECT COUNT(*) INTO v_books FROM public.library_items WHERE user_id = v_uid AND status = 'finished';
  ...
  SELECT COALESCE(MAX(best_streak),0) INTO v_habit_max FROM public.habits WHERE user_id = v_uid;
  ...
  IF EXISTS (SELECT 1 FROM public.finance_transactions WHERE user_id = v_uid AND type = 'income') AND
  ...
  IF EXISTS (SELECT 1 FROM public.xp_history WHERE user_id = v_uid AND created_at::time < '06:00' AND amount > 0) AND
     public.award_title(v_uid,'early_bird','Madrugador','XP antes das 06h','🌅','common') THEN v_awarded := v_awarded + 1; END IF;
  IF EXISTS (SELECT 1 FROM public.xp_history WHERE user_id = v_uid AND created_at::time > '22:00' AND amount > 0) AND
     public.award_title(v_uid,'night_warrior','Guerreiro Noturno','XP depois das 22h','🌙','common') THEN v_awarded := v_awarded + 1; END IF;

  RETURN v_awarded;
END $$;
GRANT EXECUTE ON FUNCTION public.check_all_titles(uuid) TO authenticated;

-- Rank auto-updater (uses same logic as client side — mirrors ranks.ts)
...
DECLARE
  ...
  v_new_rank text := 'beginner'; v_prev text;
BEGIN
  ...
  IF v_p.level >= 5 AND v_p.total_xp >= 500 AND v_skill_max >= 3 AND v_achievements >= 2 AND v_p.streak_days >= 3 AND v_missions >= 10 THEN v_new_rank := 'explorer'; END IF;
  IF v_p.level >= 10 AND v_p.total_xp >= 2000 AND v_skill_max >= 5 AND v_skill_count >= 2 AND v_achievements >= 5 AND v_p.streak_days >= 7 AND v_missions >= 30 THEN v_new_rank := 'warrior'; END IF;
  IF v_p.level >= 20 AND v_p.total_xp >= 6000 AND v_skill_max >= 8 AND v_skill_count >= 3 AND v_achievements >= 10 AND v_p.streak_days >= 14 AND v_missions >= 75 THEN v_new_rank := 'elite'; END IF;
  IF v_p.level >= 35 AND v_p.total_xp >= 15000 AND v_skill_max >= 12 AND v_skill_count >= 4 AND v_achievements >= 18 AND v_p.streak_days >= 30 AND v_missions >= 150 THEN v_new_rank := 'master'; END IF;
  IF v_p.level >= 50 AND v_p.total_xp >= 35000 AND v_skill_max >= 18 AND v_skill_count >= 5 AND v_achievements >= 30 AND v_p.streak_days >= 60 AND v_missions >= 300 THEN v_new_rank := 'legend'; END IF;
  IF v_p.level >= 75 AND v_p.total_xp >= 75000 AND v_skill_max >= 25 AND v_skill_count >= 6 AND v_achievements >= 50 AND v_p.streak_days >= 100 AND v_missions >= 600 THEN v_new_rank := 'ascended'; END IF;

  v_prev := v_p.current_rank;
  IF v_new_rank <> COALESCE(v_prev, 'beginner') THEN
    UPDATE public.profiles SET current_rank = v_new_rank, updated_at = now() WHERE id = v_uid;
    PERFORM public.record_timeline(v_uid, 'rank:'||v_new_rank, 'rank',
      'Novo rank: '||initcap(v_new_rank), 'Você subiu para '||v_new_rank, '🏆',
      jsonb_build_object('from', v_prev, 'to', v_new_rank));
    INSERT INTO public.inventory_items(user_id, item_key, kind, name, description, icon, rarity)
    VALUES (v_uid, 'rank:'||v_new_rank, 'medal', 'Rank '||initcap(v_new_rank), 'Alcançou rank '||v_new_rank, '🏆',
      CASE v_new_rank WHEN 'ascended' THEN 'mythic'::public.item_rarity
                      WHEN 'legend' THEN 'legendary'::public.item_rarity
                      WHEN 'master' THEN 'epic'::public.item_rarity
                      WHEN 'elite' THEN 'epic'::public.item_rarity
                      WHEN 'warrior' THEN 'rare'::public.item_rarity
                      ELSE 'common'::public.item_rarity END)
    ON CONFLICT DO NOTHING;
  END IF;
  RETURN v_new_rank;
END $$;
...
-- Full sync — call after any XP event
CREATE OR REPLACE FUNCTION public.sync_life_state()
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_uid uuid := auth.uid();
  v_score numeric; v_rank text; v_titles int;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'not authenticated'; END IF;
...
-- Trigger on xp_history INSERT: async-ish sync (level up detection, titles)
CREATE OR REPLACE FUNCTION public.xp_history_after_insert()
RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_p public.profiles; v_prev_level int;
BEGIN
  IF NEW.amount <= 0 THEN RETURN NEW; END IF;
  SELECT * INTO v_p FROM public.profiles WHERE id = NEW.user_id;
  IF FOUND THEN
    -- Level up milestone timeline events
    ...
    IF v_p.level >= 100 THEN PERFORM public.record_timeline(NEW.user_id, 'level:100','level','Nível 100 alcançado','Centenário.','🌌'); END IF;
    ...
    IF v_p.total_xp >= 100000 THEN PERFORM public.record_timeline(NEW.user_id, 'xp:100k','xp','100.000 XP total','Épico.','💥'); END IF;

    -- Auto titles + rank + life score (silent errors so XP insert never fails)
...
-- Auto timeline for achievement inserts
...
DECLARE
  ...
  v_pages_daily numeric; v_savings_weekly numeric;
  v_proj_xp numeric; v_proj_level int; v_proj_books int; v_proj_workouts int; v_proj_savings numeric;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'not authenticated'; END IF;
  SELECT * INTO v_p FROM public.profiles WHERE id = v_uid;
  ...
  SELECT COUNT(*)::numeric/60*7 INTO v_workouts_weekly FROM public.workouts
    WHERE user_id = v_uid AND workout_date >= CURRENT_DATE - 60;
  SELECT COALESCE(SUM(pages_read),0)::numeric/60 INTO v_pages_daily FROM public.health_logs
    WHERE user_id = v_uid AND log_date >= CURRENT_DATE - 60;
  SELECT COALESCE(SUM(CASE WHEN type='income' THEN amount ELSE -amount END),0)::numeric/60*7
    INTO v_savings_weekly FROM public.finance_transactions
    WHERE user_id = v_uid AND date >= CURRENT_DATE - 60;

  v_proj_xp := v_xp_daily * p_days;
  v_proj_level := GREATEST(1, floor(sqrt((COALESCE(v_p.total_xp,0) + v_proj_xp) / 50.0))::int + 1);
  v_proj_books := floor(v_pages_daily * p_days / 250)::int;
  v_proj_workouts := floor(v_workouts_weekly * p_days / 7)::int;
  v_proj_savings := round(v_savings_weekly * p_days / 7, 2);
  ...
  RETURN jsonb_build_object(
    ...
    'level', v_proj_level,
    'books', v_proj_books,
    'workouts', v_proj_workouts,
    'savings', v_proj_savings,
...
-- Boss weekly generator (called by cron)
...
DECLARE
  ...
  v_missions_avg int; v_workouts_avg int; v_pages_avg int; v_xp_avg int;
  v_id uuid; v_objectives jsonb; v_reward int;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'not authenticated'; END IF;
  IF EXISTS (SELECT 1 FROM public.weekly_bosses WHERE user_id = v_uid AND week_start = v_week_start) THEN
  ...
  SELECT COALESCE(COUNT(*)::int,0) INTO v_workouts_avg FROM public.workouts
    WHERE user_id = v_uid AND workout_date >= CURRENT_DATE - 28;
  SELECT COALESCE(SUM(pages_read)::int,0) INTO v_pages_avg FROM public.health_logs
    WHERE user_id = v_uid AND log_date >= CURRENT_DATE - 28;
  ...
  v_objectives := jsonb_build_array(
    ...
    jsonb_build_object('key','pages','label','Ler páginas','target', GREATEST(20, (v_pages_avg/4)+10), 'current', 0),
  ...
  VALUES (v_uid, v_week_start,
    'Boss da Semana',
    'Complete todos os objetivos até domingo para derrotá-lo.',
    '👹', v_objectives, v_reward)
  RETURNING id INTO v_id;
  RETURN v_id;
END $$;
GRANT EXECUTE ON FUNCTION public.generate_weekly_boss(uuid) TO authenticated;

-- Boss progress calculator
...
DECLARE
  ...
  v_missions int; v_workouts int; v_pages int; v_xp int; v_relapses int;
  v_obj jsonb; v_new jsonb := '[]'::jsonb; v_all_done boolean := true;
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_workouts FROM public.workouts
    WHERE user_id = v_uid AND workout_date >= v_week_start;
  SELECT COALESCE(SUM(pages_read),0) INTO v_pages FROM public.health_logs
    WHERE user_id = v_uid AND log_date >= v_week_start;
  ...
  IF v_all_done AND v_boss.status = 'active' THEN
    UPDATE public.weekly_bosses SET status = 'completed', completed_at = now(), objectives = v_new
      WHERE id = v_boss.id;
    ...
    ON CONFLICT DO NOTHING;
  ELSE
    UPDATE public.weekly_bosses SET objectives = v_new WHERE id = v_boss.id;
  END IF;
...
-- Day detail (for heatmap click)
CREATE OR REPLACE FUNCTION public.get_day_detail(p_date date)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_uid uuid := auth.uid();
  v_xp int; v_missions int; v_workouts int; v_pages int; v_sleep numeric; v_focus_min int;
BEGIN
  IF v_uid IS NULL THEN RETURN NULL; END IF;
  ...
  SELECT COUNT(*) INTO v_workouts FROM public.workouts WHERE user_id = v_uid AND workout_date = p_date;
  SELECT COALESCE(SUM(pages_read),0), COALESCE(AVG(sleep_hours),0) INTO v_pages, v_sleep
    FROM public.health_logs WHERE user_id = v_uid AND log_date = p_date;
  ...
  RETURN jsonb_build_object(
    'date', p_date, 'xp', v_xp, 'missions', v_missions, 'workouts', v_workouts,
    'pages', v_pages, 'sleep_hours', v_sleep, 'focus_minutes', v_focus_min
  );
END $$;
GRANT EXECUTE ON FUNCTION public.get_day_detail(date) TO authenticated;

-- Full stats aggregate for AI Coach + analytics
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'workouts_30d', (SELECT COUNT(*) FROM public.workouts WHERE user_id = v_uid AND workout_date >= CURRENT_DATE - 30),
    'pages_30d', (SELECT COALESCE(SUM(pages_read),0) FROM public.health_logs WHERE user_id = v_uid AND log_date >= CURRENT_DATE - 30),
    ...
    'finance_balance', (SELECT COALESCE(SUM(CASE WHEN type='income' THEN amount ELSE -amount END),0) FROM public.finance_transactions WHERE user_id = v_uid),
...
BEGIN
  ...
  FROM (
    SELECT date(created_at) AS d, SUM(amount) AS x
...
-- Backfill: run once for existing users
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN SELECT id FROM public.profiles LOOP
    BEGIN PERFORM public.check_all_titles(r.id); EXCEPTION WHEN OTHERS THEN NULL; END;
    BEGIN PERFORM public.check_rank(r.id); EXCEPTION WHEN OTHERS THEN NULL; END;
    BEGIN PERFORM public.calc_life_score(r.id); EXCEPTION WHEN OTHERS THEN NULL; END;
  END LOOP;
END $$;

-- Achievements unique index safety
DO $$ BEGIN
  CREATE UNIQUE INDEX achievements_user_badge_uniq2 ON public.achievements(user_id, badge_key);
