...
DECLARE
  ...
  s_workouts numeric; s_recovery numeric; s_focus numeric;
...
BEGIN
  ...
  v_total := (s_level + s_xp + s_streak + s_missions + s_habits + s_workouts + s_recovery + s_focus + s_sleep);
  ...
  VALUES (v_uid, CURRENT_DATE, v_total, jsonb_build_object(
    ...
    'workouts',s_workouts,'recovery',s_recovery,'focus',s_focus,'sleep',s_sleep
  ))
...
DECLARE
  ...
  v_savings_weekly numeric;
  v_proj_xp numeric; v_proj_level int; v_proj_workouts int; v_proj_savings numeric;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'not authenticated'; END IF;
  SELECT * INTO v_p FROM public.profiles WHERE id = v_uid;
...
DECLARE
  ...
  v_missions_avg int; v_workouts_avg int; v_xp_avg int;
...
DECLARE
  v_uid uuid := auth.uid();
  v_xp int; v_missions int; v_workouts int; v_sleep numeric; v_focus_min int;
BEGIN
  IF v_uid IS NULL THEN RETURN NULL; END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'sleep_hours', v_sleep, 'focus_minutes', v_focus_min
  );
END $$;
